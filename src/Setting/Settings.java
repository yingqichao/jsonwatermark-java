package Setting;

public class Settings {
    public static double DEFAULT_C = 0.1;
    public static double DEFAULT_DELTA = 0.5;
    public static int DEFAULT_EMBEDLEN = 7;
    public static int DEFAULT_EMBEDLEN_LONGMODE = 10;
    public static int DEFAULT_MINLEN = 7;
    public static int DEFAULT_MINLEN_EXCEL = 5;
    public static int DEFAULT_DATALEN = 4;
    public static int DEFAULT_DATALEN_LONGMODE = 6;
    public static String newTagName = "addw";
    public static String packageNumName = "packageNum";
    public static double max_allowed_modification = 0.01;
    public static int row_for_water_len = 20;

    public static boolean NORMAL = true;
    public static boolean LENGTH = false;
    public static boolean LONG = true;
    public static boolean SHORT = false;
    public static int reserved_space = 100;
}
